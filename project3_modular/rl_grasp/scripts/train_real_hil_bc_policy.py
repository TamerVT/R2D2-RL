...
class RealHILBCDataset(Dataset):
    ...
    Scalar features are read from the Parquet data file.
    Frames are decoded once from the MP4 and kept in RAM as uint8 HWC images.

    For the current dataset size, this is simpler and fast enough.
    """
    ...
    def __init__(
        ...
        *,
        ...
        if self.zero_motor_currents:
            # Match the current sim HIL-compatible wrapper, which exposes
            # zero placeholders for the 6 motor-current channels.
    ...
    def _decode_video_frames(self, path: Path) -> list[np.ndarray]:
        """
        Decode wrist-camera frames with system FFmpeg instead of OpenCV.

        OpenCV's bundled FFmpeg backend may fail to decode our AV1 MP4 files,
        while the system ffmpeg binary can decode them correctly.
        """
        ...
        try:
            ...
            raise RuntimeError(
                "ffmpeg binary was not found. Please install system FFmpeg."
            ) from exc
        ...
        if len(raw) % bytes_per_frame != 0:
            raise RuntimeError(
                f"Unexpected FFmpeg byte count: {len(raw)} is not divisible by "
...
def compute_normalization_stats(dataset: RealHILBCDataset, train_indices: list[int]) -> NormalizationStats:
    ...
    # Prevent division by tiny numbers, especially for fixed one-hot or zero-like channels.
...
def evaluate(
    ...
    for batch in loader:
        ...
        pred_action = (
            pred_norm_action * stats_tensors["action_std"]
            + stats_tensors["action_mean"]
        )
    ...
    # Normalize by number of samples and action dimensions.
    action_dim = 6
    denom = max(1, total_count * action_dim)
...
def parse_args() -> TrainConfig:
    ...
    parser.add_argument(
        "--output-dir",
        default="project3_modular/rl_grasp/outputs/real_hil_bc_warmstart_v1",
    )
