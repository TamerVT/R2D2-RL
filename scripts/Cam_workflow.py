...
class _Box:
    """Mimic a minimal Ultralytics Box object: b.xyxy[0] works."""
    __slots__ = ("_xyxy", "conf")

    def __init__(self, x1, y1, x2, y2, conf=1.0):
        self._xyxy = np.array([[x1, y1, x2, y2]], dtype=np.float32)
        self.conf = conf  # not used, but handy
...
class _Boxes:
    """Iterable container similar to ultralytics Results.boxes."""
    __slots__ = ("_boxes",)

    def __init__(self, boxes_xyxy):
        # boxes_xyxy: list of (x1,y1,x2,y2)
        self._boxes = [_Box(*b) for b in boxes_xyxy]
    ...
    @property
    def xyxy(self):
        """(N,4) array convenience."""
        if not self._boxes:
            return np.zeros((0, 4), dtype=np.float32)
        return np.concatenate([b.xyxy for b in self._boxes], axis=0)


class _Result:
    """Minimal result object with .boxes attribute."""
    __slots__ = ("boxes",)
...
class Net:
    """
    Drop-in-ish replacement for your YOLO Net wrapper, but backed by bbox_service.py.
    - weights is unused (kept only so your call site doesn't change).
    - imgsz: if not None, resizes before sending (keeps your 320x320 behavior).
    - conf: unused here (server already filters); kept for compatibility.
    """

    def __init__(self, weights=None, imgsz=320, conf=0.25,
    ...
    def forward(self, img_bgr):
        ...
        if self.imgsz is not None:
            img_bgr = cv2.resize(img_bgr, (self.imgsz, self.imgsz), interpolation=cv2.INTER_AREA)

        boxes_xyxy = self.client.detect(img_bgr)  # [(x1,y1,x2,y2), ...]
        return _Result(boxes_xyxy)
    ...
    while time.time() < t_start + 60:
        ...
        if cup_boxes is not None:  # bbox outputs live here [2](https://medium.com/@wenrudong/what-is-opencvs-inter-area-actually-doing-282a626a09b3)[5](https://iq.opengenus.org/different-interpolation-methods-in-opencv/)
            for b in cup_boxes:
                ...
                x1, y1, x2, y2 = b.xyxy[0]   # geometry only
                
                cls = CUP_ID                 # or define CUBE_ID and use that
                conf = 1.0                   # bbox service doesn’t provide conf
                label = f"cube {conf:.2f}"   # fixed label; you’ll do color later
